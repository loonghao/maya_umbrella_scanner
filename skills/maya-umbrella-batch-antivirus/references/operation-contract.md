...
Read this reference before executing either helper mode. Cleanup changes files and Maya state; the confirmation and verification requirements below are part of the contract, not optional suggestions.

## Helper interface

Resolve `batch_scan.py` relative to the installed `SKILL.md` and invoke it with Python on Windows. Quote every path. Each target uses its own `--path` argument.

Scan one or more targets:

```powershell
python "<skill-directory>\scripts\batch_scan.py" scan `
  --path "C:\project\scenes" `
  --path "D:\shots\asset" `
  --scanner "C:\tools\maya-umbrella\maya_umbrella.exe" `
...
```powershell
python "<skill-directory>\scripts\batch_scan.py" clean `
  --path "C:\project\scenes" `
  --path "D:\shots\asset" `
  --scanner "C:\tools\maya-umbrella\maya_umbrella.exe" `
  --maya-version 2024 `
...
Then run `scan` again against the same `--path` and `--scanner` values. Use a different `--report` path so the pre-scan, cleanup, and post-scan evidence remain independently inspectable.
...
- `--scanner` selects an exact `maya_umbrella.exe`. Omit it only when helper discovery resolves one unambiguous executable.
- `--report` writes auditable output. It is optional only for scan-only work; cleanup requires the approved scan report.
- Every `--report` destination must be a fresh `.json` path. The helper refuses to overwrite an existing report or scene file.
...
- Cleanup requires a scanner whose `--help` exposes `--approved-scan-report`. The helper forwards the report so the scanner repeats the binding check immediately before Maya starts; older scanners fail closed.
...
Scan recursively checks only `.ma` and `.mb` files for Maya Umbrella signatures and excludes the reserved `_virus` backup/quarantine directory. The name is fixed so an arbitrary existing project subtree cannot be hidden from scanning. Scan should not modify scenes, though the helper may create temporary output and the requested report.
...
2. The cleanup helper completed without an indeterminate target.
3. Expected scene backups exist and were not silently disabled.
4. The helper's final full-scope scan and a separate post-clean scan completed over the same accepted paths.
